...
public class GitServiceImpl implements GitService {
	...
	public void syncBranch(String system, String alias, String gitAddress, String branch, String versionCode) {
		...
		} catch (GitAPIException | IOException | NoSuchMethodException | IllegalAccessException
	...
	public void checkCode(String system, String branch) {
		...
		String cmd = "export JAVA_HOME=/Library/Java/JavaVirtualMachines/jdk1.8.0_40.jdk/Contents/Home"
				+ ";export PATH=$PATH:$JAVA_HOME;" + "java -jar " + firelineJarPath + " -s=" + branchCodeDir + " -r="
				+ checkReportDir;
	...
	public void genDoc() {
		...
		if(lock.tryLock()){
			try {
				ApplicationHome home = new ApplicationHome(this.getClass());
				File codeDir = new File(home.getDir(), String.format("%s", codePath));
				File docDir = new File(home.getDir(), String.format("%s/%s", apidocPath, "Doc"));
				
				if(docDir.exists()){
					logger.info(docDir.getPath() + "存在，先删除...");
					FileUtil.deleteFile(docDir);
				}
				
				logger.info("创建文件加：{}", docDir.getPath() );
				docDir.mkdir();
				GitUtil.createApisHtml(codeDir.getPath(), docDir.getPath());
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				lock.unlock();
